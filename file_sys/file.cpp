...
bool File::addContent(const char * content, string blocks[], FAT &fat)
{
	...
	for (int i = 0; i < len; i++) {
		if (blocks[index[size]].length() <= N) blocks[index[size]] += content[i];
		else {
...
{
	for (int i = 0; i<this->size; i++) fat.addBlock(index[i], blocks);
}

string File::toString(string blocks[])
{
	string s;
	for (int i = 0; i <= this->size; i++) s+=blocks[index[i]];
	return s;
}